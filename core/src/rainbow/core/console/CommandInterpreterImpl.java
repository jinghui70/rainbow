...
public class CommandInterpreterImpl implements CommandInterpreter, BundleListener {
	...
	public String getPrompt() {
		...
		return provider == null ? "rainbow" : provider.getName();
	}
	 ...
	 * 
	 * Searches the list of command providers using introspection until it finds one
	 * that contains a matching method. It searches for a method with the name
	 * "_cmd" where cmd is the command to execute. For example, for a command of
	 ...
	 * user interaction.
	 * <p>
	 * Note that this number does not include the line for the 'more' prompt itself.
	 * <p>
	 ...
	 * 
	 * @return The user provided integer or the defaultAnswer, if user provided an
	 *         empty input.
	 */
	...
	public void bundleStop(String id) {
        if (curProviderExtension == null)
            return;
        if (Objects.equals(id, curProviderExtension.getBundle()))
